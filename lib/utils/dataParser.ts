...
// Note: PDF and DOCX parsing moved to server-side API route (/api/files/parse)
// This file is kept for backwards compatibility but only supports Excel and CSV client-side
...
export async function parseFile(file: File): Promise<ParsedData> {
  ...
  if (fileType.includes('spreadsheet') || fileName.endsWith('.xlsx') || fileName.endsWith('.xls')) {
    ...
    throw new Error(`Unsupported file type: ${fileType || 'unknown'}. PDF and DOCX must be parsed via API route.`);
  }
}
